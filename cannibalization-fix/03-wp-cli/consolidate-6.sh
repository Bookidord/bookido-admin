#!/bin/bash
# CONSOLIDATE #6: software-gestion-negocios-la-vega → software-gestion-negocios-santiago
# Shared query: software negocios reservaciones pos
WP_PATH="/home/u488152486/domains/xn--gestindo-z3a.com/public_html"
cd "$WP_PATH"

LOSER_ID=$(wp post list --post_name="software-gestion-negocios-la-vega" --post_type=post --fields=ID --format=ids 2>/dev/null)
...
wp post get $LOSER_ID --field=post_content > /tmp/loser-software-gestion-negocios-la-vega-content.txt
echo "Loser content saved to /tmp/loser-software-gestion-negocios-la-vega-content.txt — review and merge manually if needed"
