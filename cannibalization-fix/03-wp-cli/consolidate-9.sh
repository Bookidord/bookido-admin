#!/bin/bash
# CONSOLIDATE #9: software-reservaciones-salon-belleza-puerto-plata → software-reservaciones-salon-belleza-santiago
...
LOSER_ID=$(wp post list --post_name="software-reservaciones-salon-belleza-puerto-plata" --post_type=post --fields=ID --format=ids 2>/dev/null)
...
wp post get $LOSER_ID --field=post_content > /tmp/loser-software-reservaciones-salon-belleza-puerto-plata-content.txt
echo "Loser content saved to /tmp/loser-software-reservaciones-salon-belleza-puerto-plata-content.txt — review and merge manually if needed"
