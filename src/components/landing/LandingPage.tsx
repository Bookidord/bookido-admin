        ...
        <div className="fixed right-4 top-4 z-50 flex flex-col gap-2">
          {landing.instagram_url && (
            <a href={landing.instagram_url} target="_blank" rel="noopener noreferrer" aria-label="Instagram"
              className="flex h-10 w-10 items-center justify-center rounded-full shadow-lg transition hover:scale-110"
          ...
          {landing.tiktok_url && (
            <a href={landing.tiktok_url} target="_blank" rel="noopener noreferrer" aria-label="TikTok"
              className="flex h-10 w-10 items-center justify-center rounded-full bg-black shadow-lg transition hover:scale-110">
              <svg className="h-5 w-5 text-white" viewBox="0 0 24 24" fill="currentColor">
          ...
          {landing.facebook_url && (
            <a href={landing.facebook_url} target="_blank" rel="noopener noreferrer" aria-label="Facebook"
              className="flex h-10 w-10 items-center justify-center rounded-full bg-[#1877F2] shadow-lg transition hover:scale-110">
              <svg className="h-5 w-5 text-white" viewBox="0 0 24 24" fill="currentColor">
        ...
        <section className="border-t border-white/[0.06] bg-zinc-900 px-6 py-20">
          <div className="mx-auto max-w-4xl">
            <AnimateIn>
              ...
              <h2 className="mb-8 text-3xl font-bold text-white">Productos disponibles</h2>
            </AnimateIn>
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {products.map((p, i) => {
                const waText = encodeURIComponent(`Hola, quiero pedir: *${p.name}* (RD$${p.price.toLocaleString("es-DO")}). ¿Está disponible?`);
                const waNumber = landing.whatsapp?.replace(/\D/g, "") ?? "";
                return (
                  <AnimateIn key={p.id} delay={i * 60}>
                    <div className="group flex flex-col rounded-2xl border border-white/[0.08] bg-zinc-950 overflow-hidden transition hover:border-white/[0.15]">
                      {p.photo_url ? (
                        <div className="h-48 overflow-hidden">
                          <img src={p.photo_url} alt={p.name} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" />
                        </div>
                      ) : (
                        <div className="flex h-48 items-center justify-center bg-white/[0.03] text-5xl">🛍️</div>
                      )}
                      <div className="flex flex-1 flex-col p-4">
                        <p className="font-semibold text-white">{p.name}</p>
                        {p.description && <p className="mt-1 text-xs leading-relaxed text-zinc-500 line-clamp-2">{p.description}</p>}
                        <div className="mt-auto pt-4 flex items-center justify-between gap-3">
                          ...
                          {waNumber && (
                            <a href={`https://wa.me/${waNumber}?text=${waText}`} target="_blank" rel="noopener noreferrer"
                              className="flex items-center gap-1.5 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-emerald-500">
