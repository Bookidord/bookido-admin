...
export default async function PanelPage() {
  ...
  if (admin) {
    ...
    // New clients this month (unique emails since month start)
    ...
    <div className="mx-auto max-w-5xl px-5 py-8 lg:px-8 lg:py-10">
          ...
          upcomingBookings.length > 0 ? (
            <div>
              <div className="px-5 pt-5 pb-2">
                <p className="text-xs text-zinc-500">No hay citas para hoy. Próximas reservas:</p>
              </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:flex-shrink-0">
                    {b.customer_phone && (
                      <a
                        href={`https://wa.me/${b.customer_phone.replace(/\D/g, "")}?text=${encodeURIComponent(`Hola ${b.customer_name}, te recordamos tu cita a las ${format(start, "HH:mm")}.`)}`}
                        target="_blank"
      ...
      <div className="mt-6 rounded-xl border border-white/[0.07] bg-ink-900/40 p-5">
        ...
        <p className="mt-3 text-xs text-zinc-600">
          {statsWeek} {statsWeek === 1 ? "cita confirmada" : "citas confirmadas"} esta semana
        </p>
      </div>

      {/* Quick actions */}
      <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
        {/* Nueva cita manual — primary */}
        <Link
